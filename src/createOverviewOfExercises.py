...
def generated_tree_expression(list_of_filenames, number_of_exercises_in_files):
    output = ""
    last_path = []
    for filename in list_of_filenames:
        ...
        for val in get_all_sub_paths(filename):
            if not is_new_path_included_in_the_reference_path(last_path, val):
                number_of_exercises = number_of_exercises_in_files["/".join(val)]
...
def main():
    ...
    exercise_tree = generated_tree_expression(latex_files, number_of_exercises_in_files)
    print(exercise_tree)
